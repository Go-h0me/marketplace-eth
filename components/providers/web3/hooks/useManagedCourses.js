...
// cai nay se phan tan goc cac account trong metamask
export const handler = (web3, contract) => (account) => {
  const swrRes = useSWR(
    ...
    async () => {
      const courses = [];
      //  FETCH MANAGED COURSES trong course marketplace
      const courseCount = await contract.methods.getCourseCount().call();

      // nhan nhung khoa hoc moi nhat va  dau tien cua khoa hoc
      //  ys la  5 -courseCount
      // con 4 utuc la khoa hoc cu roi the thoi
      // vong lap nay dao nguoc
